...
export default function CapacityAnalysis({ data, demands, resources, processCycles = [], settings, filters, onFiltersChange }: Props) {
  ...
  const handleExportOverview = async () => {
    ...
    try {
      ...
      const detailHeaderRow = ['', '', ...allMonths];
      ...
      peakHeader.font = { bold: true, color: { argb: 'FFFFFFFF' } };
      peakHeader.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF4338CA' } }; // Indigo 700
      peakHeader.alignment = { horizontal: 'center', vertical: 'middle' };
      ...
      allTeams.forEach(team => {
        ...
        peakDemands.forEach((demand, index) => {
          ...
          const row = peakSheet.addRow([
            ...
            Number(demand.demandHours.toFixed(1)),
            `${ratio.toFixed(1)}%`
          ]);
      ...
      anchor.download = `${today}_产能负荷分析总览.xlsx`;
      anchor.click();
  ...
  const handleExport = async () => {
    ...
    const addHeatmapSheet = (category: 'human' | 'machine') => {
      ...
      const sheetName = category === 'human' ? '人员负荷' : '设备负荷';
      const worksheet = workbook.addWorksheet(sheetName);
      
      // Header Row
      const headerRow = ['班组 \\ 月份', ...months];
      const header = worksheet.addRow(headerRow);
      header.font = { bold: true, color: { argb: 'FFFFFFFF' } };
      header.fill = { 
        type: 'pattern', 
    ...
    if (viewMode === 'heatmap') {
      ...
      top10PartHours.forEach(teamData => {
        teamData.data.forEach((item, index) => {
      ...
      const exportTeams = sortTeams(Array.from(new Set(activeData.map(d => d.team))), settings?.teamOrder, settings?.teamCategories);

      // Header Row
      const headerRow = ['', '', ...exportMonths];
      const header = worksheet.addRow(headerRow);
      header.font = { bold: true, color: { argb: 'FFFFFFFF' } };
    ...
    anchor.href = url;
    const today = new Date().toISOString().split('T')[0];
    anchor.download = `${getChartTitle()}_${today}.xlsx`;
    anchor.click();
    window.URL.revokeObjectURL(url);
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex bg-slate-100 p-1 rounded-xl w-fit shadow-inner">
          ...
          <button 
            ...
            className={cn("px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all", viewMode === 'top10' ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700")}
          >
            <ListOrdered size={18} /> 料号工时排名Top10
          </button>
        </div>
